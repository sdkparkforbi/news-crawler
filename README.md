...
| ⑨ DB 업로드(선택) | `DB업로드=True` + `.env`(DB_*)면 기사·임베딩 4종을 MySQL에 적재(`articles` + `article_embeddings`·`article_noun_embeddings`·`article_emb_oai`·`article_noun_emb_oai`) |

> 키가 없는 임베딩은 자동으로 건너뜁니다. 키는 노트북에서 `getpass`로 입력받아 **메모리에만** 두며 파일·로그에 남기지 않습니다. (분석 노트북은 `build_analysis.py` 로 생성)
>
> **환경변수/`.env`로 설정 가능** — `MIDDLETON_URL`·`MIDDLETON_API_KEY`·`OPENAI_API_KEY`·`BGE_MODEL`·`OAI_EMB_MODEL`·`CLASSIFY_MODEL`, 그리고 DB 업로드용 `DB_HOST`·`DB_PORT`·`DB_USER`·`DB_PASSWORD`·`DB_NAME`. `.env.example`을 `.env`로 복사해 채우면 노트북이 자동으로 읽습니다(키가 환경변수에 있으면 `getpass` 프롬프트가 생략됨). 자체 임베딩 서버를 쓰면 `MIDDLETON_URL`만 바꾸면 됩니다. `.env`는 `.gitignore`에 포함되어 커밋되지 않습니다.
>
> **DB 업로드**: 설정 셀에서 `DB업로드 = True` 로 두고 `.env` 에 `DB_HOST`/`DB_USER`/`DB_PASSWORD`(필요시 `DB_PORT`·`DB_NAME`)를 넣으면, 12단계에서 기사와 4종 임베딩이 MySQL에 적재됩니다(테이블은 없으면 자동 생성). 미설정 시 로컬 분석만 수행합니다.
