...
### A. Google Colab
1. `notebooks/뉴스크롤러.ipynb` 를 Colab에 업로드
2. (이 repo가 private면) Colab Secrets 🔑 에 `GH_TOKEN`(GitHub 토큰) 등록 + *노트북 액세스* ON
3. 위에서부터 Run — 1번 셀이 repo를 클론하고, 구글드라이브에 결과를 저장합니다
...
- 비밀키는 코드에 두지 말 것. 수집/추출은 키가 필요 없고, private repo 클론 토큰만
  Colab Secrets(`GH_TOKEN`) 또는 `.env`(`.env.example` 참고)로 주입합니다.
